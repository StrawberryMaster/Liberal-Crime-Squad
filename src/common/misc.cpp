...
#ifndef DONT_INCLUDE_SDL
const array<const char*, MUSIC_OFF> MUSIC_FILES = {{
    "titlemode", "newgame", "basemode", "siege", "activate", "sleepers",
    "stopevil", "reviewmode", "liberalagenda", "disbanded", "finances",
    "cartheft", "elections", "shopping", "sitemode", "suspicious", "alarmed",
    "heavycombat", "defense", "conquer", "carchase", "footchase", "interrogation",
    "trial", "recruiting", "dating", "newspaper", "lacops", "newscast", "glamshow",
    "anchor", "abort", "victory", "defeat", "reaganified", "stalinized"
}};

void MusicClass::init()
{
    if (songsinitialized) return;

    // initialize SDL audio
    if (SDL_Init(SDL_INIT_AUDIO) != 0) {
        gamelog.log(string("SDL init failed: ") + SDL_GetError());
        pressAnyKey();
        exit(EXIT_FAILURE);
    }
    ...
    if (Mix_OpenAudio(44100, MIX_DEFAULT_FORMAT, 2, 4096) != 0) {
        gamelog.log(string("Mixer init failed: ") + Mix_GetError());
        SDL_Quit();
        pressAnyKey();
        exit(EXIT_FAILURE);
    }

    oggsupport = (Mix_Init(MIX_INIT_OGG) & MIX_INIT_OGG) != 0;
    if (!oggsupport) {
        gamelog.log("OGG support failed, using MIDI fallback");
    }

    // batch load all music files
    eraseAlt();
    mvaddstrAlt(12, 0, "Loading music...");
    refreshAlt();

    const string base_path = string(artdir) + (oggsupport ? "ogg/" : "midi/");
    const string ext = oggsupport ? ".ogg" : ".mid";

    for (int i = 0; i < MUSIC_OFF; ++i) {
        const string path = base_path + MUSIC_FILES[i] + ext;
        songs[i] = Mix_LoadMUS(path.c_str());
        
        // if OGG fails, fallback to MIDI
        if (!songs[i] && oggsupport) {
            const string midi_path = string(artdir) + "midi/" + MUSIC_FILES[i] + ".mid";
            songs[i] = Mix_LoadMUS(midi_path.c_str());
        }

        if (!songs[i]) {
            gamelog.log(string("Failed to load: ") + path + " - " + Mix_GetError());
        }
    }
